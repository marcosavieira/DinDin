...
function Home(){
  ...
  function handleDeleteModal(id){
    ...
    if(id === idToDelete){
      return <div className="modal-excluir-task">
  ...
  return (
    <div className='home-main'>
      ...
      <div className='profile'>
        <button id='buttonProfile' name={profile} onClick={(e) => openDataModalProfile(profile)}>
        <img src={Profile} alt="" />
        </button>
        
        <span>{flag ? nome : nomePtofile}</span>
        <Link to='/sign-in'>
        <img src={LogOff} alt="" onClick={() => clearDataLocalStorage()}/>
      ...
      <div className='nav-home'>
        ...
        <div className='container-button-cadastrar-produto'>
              <button id='cadastrar-produto' name={add} onClick={(e) => openDataModal(e.target.name)}>Adicionar registro</button>
            </div>
      </div>
      <div className='nav-left'>
      <button className='container-filtro' onClick={() => handleFilter()}>
      ...
      {showFiltros && <Filtros setTransactions={setTransactions} HandleTransacoes={HandleTransacoes} />}
      
      
      <div className='container-headers'>
        <div className='header-data' onClick={() => handleOrder()}>
       ...
       {transactions.map((transaction) => {
        ...
        const dataFormat = new Date(transaction.data).toLocaleDateString('pt-br', options).split("-feira");
        const diaSemana = dataFormat[0].charAt(0).toUpperCase() + dataFormat[0].slice(1);
        const dia = new Date(transaction.data).toLocaleDateString('pt-br', {year: "numeric",
        month: "numeric",
        day: "numeric",});
        ...
        return (
          ...
          <ul name='tasks' key={transaction.id} >
            ...
            <li id={transaction.tipo === "entrada" ? "entrada-saldo" : "saida-saldo"}>
              {Number(transaction.valor / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </li>
            <div className="container-edit-exclui">
            <button id="editar" name={edit} onClick={(e) => { handleEdit(transaction.id); openDataModal(e.target.name); setIdToEdit(transaction.id)}}>
                <img src={Edit} name={edit}  alt="" />
              </button>
              <button id="excluir"  onClick={(e) => {handleDelete(transaction.id); setIdToDelete(transaction.id)  } }>
                <img src={Excluir}  alt="" />
              </button>
              {showModal && handleDeleteModal(transaction.id)}
            </div>
          </ul>
      )})}
      </div>
